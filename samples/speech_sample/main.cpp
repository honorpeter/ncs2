    ...
    try {
        ...
        for (uint32_t utteranceIndex = 0; utteranceIndex < numUtterances; ++utteranceIndex) {
            ...
            for (uint32_t frameIndex = 0; frameIndex < numFrames; frameIndex += batchSize) {
                ...
                if (!FLAGS_r.empty()) {
                    CompareScores(ptrOutputBlob->buffer().as<float *>(),
                           &ptrReferenceScores[frameIndex * numFrameElementsReference * numBytesPerElementReference],
                           &frameError,
                           numFramesThisBatch,
                           numFrameElementsReference);
                ...
                if (FLAGS_pc) {
                    ...
                    getPerformanceCounters(request, callPerfMap);
            ...
            if (FLAGS_pc) {
                // print
                printPerformanceCounters(utterancePerfMap, calls_number, std::cout);
            }
            if (!FLAGS_r.empty()) {
